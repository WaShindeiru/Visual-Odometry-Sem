...
class VisualOdometry():
    ...
    def _load_calib(filepath):
        ...
        with open(filepath, 'r') as f:
            ...
            K = P[0:3, 0:3]
            print(K)
            # print(f"Intrinsic parameters: {K}")
    ...
    def _load_poses(filepath):
        ...
        with open(filepath, 'r') as f:
            ...
            # print(f"Pose shape: {poses[0].shape}")
            # print(f"Single pose: {poses[0]}")
        return poses
    ...
    def get_matches(self, i):
        ...
        # print(f"Keypoints1 dimension: {type(keypoints1)}")
        # print(f"Keypoints1: {keypoints1}")
        #
        # print(f"Descriptors1 dimension: {type(descriptors1)}")
        # print(f"Descriptors1: {descriptors1}")

        matches = self.flann.knnMatch(descriptors1, descriptors2, k=2)
        # store all the good matches as per Lowe's ratio test.

        # print(f"Flann matches type: {type(matches)}")
        # print(f"Flann matches: {matches}")
        # print(f"First Flann match type: {type(matches[0])}")
        # print(f"First Flann match: {matches[0]}")
        # print(f"Match distance: {matches[0][0].distance}")
...
def plot_path(ground_truth_path, predicted_path):
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(10, 5))

    gt_numpy = np.array(ground_truth_path)
    pt_numpy = np.array(predicted_path)

    ax1.plot(gt_numpy[:, 0], gt_numpy[:, 1], label="ground truth")
    ax1.plot(pt_numpy[:, 0], pt_numpy[:, 1], label="prediction")
    ax1.set_xlim((-10, 10))

    error = np.linalg.norm(gt_numpy - pt_numpy, axis=1)
    ax2.plot(np.arange(0, error.shape[0], 1), error)
...
def main():
    ...
    for i, gt_pose in enumerate(tqdm(vo.gt_poses, unit="pose")):
        if i == 0:
            ...
            # print(f"Translation vector: {cur_pose[:, 3]}")
